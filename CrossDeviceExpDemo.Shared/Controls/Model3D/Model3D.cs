...
namespace Microsoft.Toolkit.Uwp.UI.Controls
{
    ...
    public sealed class Model3D : Control
    {
        ...
        private async Task UpdateCameraAsync()
        {
            ...
            var alpha = GetRadianFromDegree(AlphaInDegrees);
            var beta = GetRadianFromDegree(BetaInDegrees);
        ...
        private double GetRadianFromDegree(double degrees)
        {
            return degrees / 180d * Math.PI;
        }

        private double GetDegreeFromRadian(double radians)
        {
            var value = (radians * 180d / Math.PI) % 360f;

            if (value < 0)
            {
                value += 360f * Math.Ceiling(Math.Abs(value / 360.0f));
            }
        ...
        /// <summary>
        /// Gets or sets a double value indicating the alpha angle in degrees (which defines the rotation on horizontal plane)
        /// </summary>
        public double AlphaInDegrees
        {
            get { return (double)GetValue(AlphaInDegreesProperty); }
        ...
        public static readonly DependencyProperty AlphaInDegreesProperty =
            DependencyProperty.Register("AlphaInDegrees", typeof(double), typeof(Model3D), new PropertyMetadata(90f, OnCameraValueChanged));

        /// <summary>
        /// Gets or sets a double value indicating the beta angle in degrees (which defines the rotation on vertical plane)
        /// </summary>
        public double BetaInDegrees
        {
            get { return (double)GetValue(BetaInDegreesProperty); }
        ...
        public static readonly DependencyProperty BetaInDegreesProperty =
            DependencyProperty.Register("BetaInDegrees", typeof(double), typeof(Model3D), new PropertyMetadata(90f, OnCameraValueChanged));

        /// <summary>
        /// Gets or sets a double value between 0 and 100 indicating the distance from the center of the scene
        /// </summary>
        ...
        public static readonly DependencyProperty RadiusPercentageProperty =
            DependencyProperty.Register("RadiusPercentage", typeof(double), typeof(Model3D), new PropertyMetadata(5f, OnRadiusValueChanged));
